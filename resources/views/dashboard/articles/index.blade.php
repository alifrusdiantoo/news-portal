<x-dashboard>
	...
	<section class="bg-gray-50 dark:bg-gray-900 antialiased">
		<div class="mx-auto max-w-screen px-4 lg:px-4">
			<!-- Start coding here -->
			<div class="bg-white dark:bg-gray-800 min-h-screen relative shadow-md sm:rounded-lg overflow-hidden">
				<div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
					<div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
						<button type="button" class="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
							<a href="/dashboard/articles/create" class="flex items-center justify-center">
						...
						<tbody>
							...
							<tr class="border-b dark:border-gray-700">
								...
								<td class="px-4 py-3 flex items-center justify-end">
									<button id="{{ $article->author->name}}-{{ $loop->iteration }}" data-dropdown-toggle="{{ $article->author->name}}-{{ $loop->iteration }}-dropdown" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
										<svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
											<path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
										</svg>
									</button>
									<div id="{{ $article->author->name}}-{{ $loop->iteration }}-dropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
										<ul class="py-1 text-sm" aria-labelledby="{{ $article->author->name . $loop->iteration }}">
											<li>
												<button type="button" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
													<a href="/dashboard/articles/{{ $article->id }}/edit" class="flex w-full items-center">
											...
											<li>
												<button type="button" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
													<a href="/dashboard/articles/{{ $article->id }}" class="flex w-full items-center">
														<svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
											...
											<li>
												<button onclick="openDeleteModal('{{ $article->id }}')" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
	...
	<!-- Delete modal -->
	<div id="delete-article-modal" class="fixed inset-0 hidden bg-gray-800 bg-opacity-75 z-50 flex items-center justify-center">
		<div class="relative p-4 w-full max-w-md max-h-full">
			<!-- Modal content -->
			<div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
				<button onclick="closeModal('delete-article-modal')" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
				...
				<p class="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want to delete this article (ID: <span id="article-id"></span>)?</p>
				<div class="flex justify-center items-center space-x-4">
					...
					<button onclick="closeModal('delete-article-modal')" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">Cancel</button>
				</div>
			</div>
		</div>
	</div>
	...
	<script>
		...
		function openDeleteModal(id) {
			document.getElementById('delete-article-form').action = '/dashboard/articles/' + id;
			document.getElementById('article-id').innerHTML = id;
			openModal('delete-article-modal');
		}
	</script>
</x-dashboard>